...
const FINAL_SUPABASE_ANON_KEY = SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY;
...
// Session management
let currentSession: { access_token: string; refresh_token: string; user: User } | null = null;

// Helper function to make authenticated API calls using axios
...
export const authApi = {
  async getSession(): Promise<{ access_token: string; refresh_token: string; user: User } | null> {
    try {
  ...
  async signIn(email: string, password: string): Promise<{ data: any; error: any }> {
    try {
      const response = await axios.post(
        `${FINAL_SUPABASE_URL}/auth/v1/token?grant_type=password`,
        { email, password },
        {
          headers: {
            'apikey': FINAL_SUPABASE_ANON_KEY,
  ...
  async signUp(email: string, password: string, metadata?: any): Promise<{ data: any; error: any }> {
    try {
      const response = await axios.post(
        `${FINAL_SUPABASE_URL}/auth/v1/signup`,
        {
          email,
          password,
          data: metadata,
  ...
  async resetPasswordForEmail(email: string): Promise<{ error: any }> {
    try {
      await axios.post(
        `${FINAL_SUPABASE_URL}/auth/v1/recover`,
        { email },
        {
          headers: {
            'apikey': FINAL_SUPABASE_ANON_KEY,
...
class QueryBuilder {
  ...
  async execute(): Promise<{ data: any; error: any }> {
    try {
      ...
      if (!response.ok) {
        console.error('Response not OK. Status:', response.status, 'Data:', response.data);
        return { data: null, error: response.data || { message: 'Request failed', status: response.status } };
      }
...
export const testSupabaseConnection = async (): Promise<boolean> => {
  try {
    const session = await authApi.getSession();
    // If we can get a session (even if null), the connection is working
...
export const verifyDeviceCode = async (
  ...
  try {
    ...
    if (error) {
      console.error('Error from Supabase REST API:', JSON.stringify(error));
      return {
        success: false,
